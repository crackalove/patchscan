...
def export_matches_for_tools(
    ...
    def _auto_color(sc: int) -> Optional[str]:
        if color_mode != "auto":
            return None
        if sc >= 95:
            return "7CFF7C"
    ...
    for m in sorted(matches, key=lambda x: (-x.score, x.a_key)):
        ...
        comment = f"PatchScan: score={m.score} reason={m.reason} src=0x{af.offset:x}"
        color = _auto_color(int(m.score))
...
def export_ida_rename_script(
    ...
    def _ida_bgr_from_rgb_hex(h: str) -> Optional[int]:
        try:
            s = h.strip().lstrip("#")
            if len(s) != 6:
                return None
    ...
    ida_color = _ida_bgr_from_rgb_hex(color_hex) if color_hex else None

    lines: List[str] = []
    lines.append("""PatchScan: применение результатов сопоставления в IDA.

Скрипт переименовывает функции в базе новой версии.
Опционально добавляет комментарий и ставит цвет функции (если задан).
Запуск: File -> Script file... или Alt+F7.
...
""")
    ...
    lines.append("    ida_bytes = None")
    lines.append("")
    lines.append("renamed = 0")
    lines.append("skipped = 0")
    lines.append("commented = 0")
    lines.append("colored = 0")
    lines.append("")
    ...
    for m in sorted(matches, key=lambda x: (-x.score, x.a_key)):
        ...
        new_name = _ida_safe_name(prefix + af.name)
        cmt = f"PatchScan: score={m.score} reason={m.reason} src=0x{af.offset:x}"
        ...
        if add_comments:
            lines.append("try:")
            lines.append(f"    idc.set_func_cmt(ea, {cmt!r}, 0)")
            lines.append("    commented += 1")
            lines.append("except Exception:")
            lines.append("    pass")

        if ida_color is not None:
            lines.append("try:")
            lines.append(f"    idc.set_color(ea, idc.CIC_FUNC, {ida_color})")
            lines.append("    colored += 1")
            lines.append("except Exception:")
            lines.append("    pass")

        lines.append("")

    lines.append("ida_kernwin.msg(f'[PatchScan] renamed={renamed} skipped={skipped} commented={commented} colored={colored}\\n')")
    return "\n".join(lines)


def export_ghidra_apply_script(json_path: str) -> str:
    """
    Генерирует Ghidra-скрипт, который применяет переименование/комментарий/цвет
    из JSON (выгрузка PatchScan).

    Запуск в Ghidra:
      Window -> Script Manager -> Run
    """
    return f'''"""PatchScan: применение результатов сопоставления в Ghidra.

Скрипт читает JSON, переименовывает функции по адресу, ставит комментарий и цвет.
Запуск: Script Manager -> Run.
...
for it in items:
    ...
    try:
        if color_hex:
            c = parse_rgb_hex(color_hex)
            if c is not None:
                cu = listing.getCodeUnitAt(addr)
                if cu is not None:
                    cu.setBackgroundColor(c)
                    colored += 1
    except Exception:
        pass

print("[PatchScan] renamed={{}} commented={{}} colored={{}}".format(renamed, commented, colored))
'''
...
def parse_args(argv: List[str]) -> argparse.Namespace:
    ...
    p.add_argument("--ida-no-comment", action="store_true", help="Не ставить комменты в IDA")
    p.add_argument("--ida-color", type=str, default=None, help="Цвет функций в IDA (RGB hex, например FFCC00)")
    p.add_argument("--ghidra-script", action="store_true", help="Сгенерить Ghidra-скрипт для применения rename/comment")
    p.add_argument("--ghidra-path", default=None, help="Куда сохранить Ghidra-скрипт (по умолчанию <out>_ghidra_apply.py)")
    p.add_argument("--export", action="store_true", help="Сохранить matches в JSON для IDA/Ghidra")
    p.add_argument("--export-path", default=None, help="Путь для export JSON (по умолчанию <out>_matches.json)")
...
def main(argv: List[str]) -> int:
    ...
    try:
        ...
        if args.ida_script:
            script = export_ida_rename_script(matches, a_funcs, b_funcs, args.ida_threshold, args.ida_prefix, add_comments=(not args.ida_no_comment), color_hex=args.ida_color)
        ...
        if getattr(args, "export", False) or getattr(args, "ghidra_script", False):
            export_path = args.export_path or (out_prefix + "_matches.json")
            items = export_matches_for_tools(
                matches,
                a_funcs,
                b_funcs,
                threshold=getattr(args, "ida_threshold", 90),
                prefix=getattr(args, "ida_prefix", ""),
            )
            with open(export_path, "w", encoding="utf-8") as f:
                json.dump(items, f, ensure_ascii=False, indent=2)
            if not args.quiet:
                logger.info(f"[*] Записал {export_path}")

        if getattr(args, "ghidra_script", False):
            gh_script = export_ghidra_apply_script(export_path)
            gh_path = out_prefix + "_ghidra_apply.py"
            with open(gh_path, "w", encoding="utf-8") as f:
                f.write(gh_script)
            if not args.quiet:
                logger.info(f"[*] Записал {gh_path}")
