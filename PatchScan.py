...
def parse_args(argv: List[str]) -> argparse.Namespace:
    ...
    p.add_argument("--html", action="store_true", help="Сохранить HTML-отчёт")
    ...
    p.add_argument("--no-banner", action="store_true", help="Не выводить стартовый баннер")
...
def main(argv: List[str]) -> int:
    ...
    try:
        ...
        if args.json or (not args.json and not args.html):
            j = match_summary_json(matches, a_funcs, b_funcs, ua, ub)
